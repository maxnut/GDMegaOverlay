...
void HitboxNode::drawForObject(gd::GameObject* obj) {
	if (obj->m_nObjectID != 749 && obj->getType() == gd::GameObjectType::kGameObjectTypeDecoration && !hacks.showDecorations) return;
	if (!obj->m_bActive) return;
	...
	switch (obj->getType()) {
		...
		case gd::GameObjectType::kGameObjectTypeDecoration:
			if(hacks.showDecorations) color = ccc4(127, 127, 255, 255);
			switch (obj->m_nObjectID) {
		        case 749:
		        	color = ccc4(255, 127, 0, 255);
					break;
		        default:
		           if(!hacks.showDecorations) return;
			}
			break;
