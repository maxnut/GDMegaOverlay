...
namespace Hacks
{
    ...
    template<class T>
    T Read(uint32_t vaddress) { T buf; return ReadProcessMemory(GetCurrentProcess(), reinterpret_cast<void*>(vaddress), &buf, sizeof(T), NULL) ? buf : T(); }
    ...
    {
        if (active) writeBytes(gd::base + 0x20A23C, {0xe9, 0x79, 0x06, 0x00, 0x00});
        else writeBytes(gd::base + 0x20A23C, {0x6a, 0x14, 0x8b, 0xcb, 0xff});
    }

    static void nospikes(bool active)
    {
        if (active) writeBytes(gd::base + 0x205347, {0x75});
        else writeBytes(gd::base + 0x205347, {0x74});
    }

    static void nohitbox(bool active)
    {
        if (active) writeBytes(gd::base + 0x20456D, {0xB8, 0x07, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x20456D, {0x8B, 0x83, 0x00, 0x03, 0x00, 0x00});
    }

    static void nosolid(bool active)
    {
        if (active) writeBytes(gd::base + 0x20456D, {0xB8, 0x14, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x20456D, {0x8B, 0x83, 0x00, 0x03, 0x00, 0x00});
    }

    static void forceblocktype(bool active)
    {
        if (active) writeBytes(gd::base + 0x20456D, {0x31, 0xC0, 0x83, 0x7B, 0x34, 0x00, 0xBA, 0x07, 0x00, 0x00, 0x00, 0x0F, 0x44, 0xC2, 0x90});
        else writeBytes(gd::base + 0x20456D, {0x8B, 0x83, 0x00, 0x03, 0x00, 0x00, 0x83, 0xF8, 0x07, 0x0F, 0x84, 0x7F, 0x0A, 0x00, 0x00});
    }

    static void everythinghurts(bool active)
    {
        if (active) writeBytes(gd::base + 0x20456D, {0xB8, 0x02, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x20456D, {0x8B, 0x83, 0x00, 0x03, 0x00, 0x00});
    }

    static void freezeplayer(bool active)
    {
        if (active) writeBytes(gd::base + 0x203519, {0x90, 0x90, 0x90});
        else writeBytes(gd::base + 0x203519, {0x50, 0xFF, 0xD6});
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x2060D9, {0xEB, 0x4A});
        else writeBytes(gd::base + 0x2060D9, {0x74, 0x4A});
    }

    static void disablerespawnblink(bool active)
    {
        if (active) writeBytes(gd::base + 0x1EF358, {0xE9, 0xBD, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x1EF358, {0x8B, 0x80, 0x64, 0x01, 0x00, 0x00});
    }

    static void disabledeatheffect(bool active)
    {
        if (active) writeBytes(gd::base + 0x1EFBE0, {0xC3});
        else writeBytes(gd::base + 0x1EFBE0, {0x55});
    }

    static void IgnoreESC(bool active)
    {
        if (active) writeBytes(gd::base + 0x1E644C, {0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(gd::base + 0x1E644C, {0xE8, 0xBF, 0x73, 0x02, 0x00});
    }

    static void InstantComplete(bool active)
    {
        if (active) writeBytes(gd::base + 0x20350D, {0xC7, 0x81, 0x7C, 0x06, 0x00, 0x00, 0x20, 0xBC, 0xBE, 0x4C, 0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(gd::base + 0x20350D, {0x8B, 0x35, 0xE0, 0x23, 0x68, 0x00, 0x81, 0xC1, 0x7C, 0x06, 0x00, 0x00, 0x50, 0xFF, 0xD6});
    }
    ...
    {
        if (active) writeBytes(libcocosbase + 0xAEDCC, {0xE9, 0x69, 0x02, 0x00, 0x00, 0x90});
        else writeBytes(libcocosbase + 0xAEDCC, {0x0F, 0x84, 0x68, 0x02, 0x00, 0x00});
    }

    static void trailon(bool active)
    {
        if (active) writeBytes(libcocosbase + 0xAEDCC, {0x90, 0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(libcocosbase + 0xAEDCC, {0x0F, 0x84, 0x68, 0x02, 0x00, 0x00});
    }

    static void inversed(bool active)
    {
        if (active) writeBytes(libcocosbase + 0xAEDCC, {0x0F, 0x85, 0x68, 0x02, 0x00, 0x00});
        else writeBytes(libcocosbase + 0xAEDCC, {0x0F, 0x84, 0x68, 0x02, 0x00, 0x00});
    }
    ...
    static void trailType(int type)
    {
        ...
        switch(type)
        {
            case 1: trailoff(true); break;
            case 2: trailon(true); break;
            case 3: inversed(true); break;
            default: break;
        }
    }
    ...
    static void hitboxType(int type)
    {
        ...
        switch(type)
        {
            case 1: nospikes(true); break;
            case 2: nohitbox(true); break;
            case 3: nosolid(true); break;
            case 4: forceblocktype(true); break;
            case 5: everythinghurts(true); break;
            default: break;
        }
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x1E8153, {0x8B, 0x86, 0x14, 0x05, 0x00, 0x00, 0xC6, 0x80, 0x30, 0x01, 0x00, 0x00, 0x01, 0xEB, 0x2A});
        else writeBytes(gd::base + 0x1E8153, {0x8B, 0x81, 0x3C, 0x02, 0x00, 0x00, 0x2B, 0x81, 0x40, 0x02, 0x00, 0x00, 0x83, 0xF8, 0x0F});
    }
    ...
    {
        if (active) writeBytes(libcocosbase + 0xB8ED6, {0x00});
        else writeBytes(libcocosbase + 0xB8ED6, {0x01});
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x188CE1, {0xE9, 0x8A, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x188CE1, {0x0F, 0x8E, 0x89, 0x00, 0x00, 0x00});
    }

    static void TextLength(bool active)
    {
        if (active) writeBytes(gd::base + 0x21ACB, {0xEB, 0x04});
        else writeBytes(gd::base + 0x21ACB, {0x7C, 0x04});
    }

    static void FilterBypass(bool active)
    {
        if (active) writeBytes(gd::base + 0x21A99, {0x90, 0x90});
        else writeBytes(gd::base + 0x21A99, {0x75, 0x04});
    }
    ...
    {
        if(active) writeBytes(libcocosbase + 0xA5424, {0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(libcocosbase + 0xA5424, {0xF3, 0x0F, 0x10, 0x45, 0x08});
    }

    static void noGlow(bool active)
    {
        if(active) writeBytes(gd::base + 0xCFF2D, {0xE9, 0xFD, 0x01, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0xCFF2D, {0x8A, 0x80, 0x89, 0x02, 0x00, 0x00});
    }

    static void fastalttab(bool active)
    {
        if(active) writeBytes(gd::base + 0x3D02E, {0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(gd::base + 0x3D02E, {0x8B, 0x03, 0x8B, 0xCB, 0xFF, 0x50, 0x18});
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x204F10, {0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90});
        else writeBytes(gd::base + 0x204F10, {0x80, 0xBE, 0x95, 0x04, 0x00, 0x00, 0x00, 0x0F, 0x85, 0xDE, 0x00, 0x00, 0x00});
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x1E4A32, {0x90, 0x90});
        else writeBytes(gd::base + 0x1E4A32, {0x75, 0x6C});
    }

    static void testmodebypass(bool active)
    {
        if (active) writeBytes(gd::base + 0x1FD270, {0xE9, 0xB7, 0x00, 0x00, 0x00, 0x90});
        else writeBytes(gd::base + 0x1FD270, {0x0F, 0x84, 0xB6, 0x00, 0x00, 0x00});
    }

    static void hideui(bool active)
    {
        if (active) writeBytes(gd::base + 0x8720A, {0xB3, 0x00, 0x90});
        else writeBytes(gd::base + 0x8720A, {0x0F, 0x44, 0xD9});
    }
    ...
    {
        if (active) writeBytes(gd::base + 0x71D48, {0xEB});
        else writeBytes(gd::base + 0x71D48, {0x74});
    }
